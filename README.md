...
- **Backend (BaaS)**: 
    - **Supabase**: Provides Auth, Database, and Realtime subscriptions.
    - **Cloudflare R2**: S3-compatible object storage for documents and certificates.
- **Compute (Serverless)**: 
...
2.  **Secure Storage**:
    - Documents and certificates are stored in **Cloudflare R2** (S3-compatible object storage).
    - Accessed only via short-lived **Signed URLs** generated by the AWS S3 SDK.
